...
fn add_rule() {
    ...
    let new_rule = Rule {
        ...
        protocol,
        source_ip: if source_ip.is_empty() {
            None
        } else {
            Some(source_ip)
        },
        destination_ip: if destination_ip.is_empty() {
            None
        } else {
            Some(destination_ip)
        },
        source_port: if source_port == 0 {
            None
        } else {
            Some(source_port)
        },
...
fn update_iptables(rule: &Rule, action: &str) {
    ...
    let source_ip = rule
        .source_ip
        .as_ref()
        .map_or("".to_string(), |ip| format!("--source {}", ip));
    let destination_ip = rule
        .destination_ip
        .as_ref()
        .map_or("".to_string(), |ip| format!("--destination {}", ip));
    let source_port = rule
        .source_port
        .map_or("".to_string(), |port| format!("--sport {}", port));
    let destination_port = rule
        .destination_port
        .map_or("".to_string(), |port| format!("--dport {}", port));
    let target = if action == "block" { "DROP" } else { "ACCEPT" };

    // Construct the iptables command as a string
    let iptables_command = format!(
        "sudo iptables -A INPUT -p {} {} {} {} {} -j {} -m comment --comment {}",
    ...
    if output.status.success() {
        ...
        eprintln!(
            "Failed to update rule in iptables. Error: {}",
            stderr_output
        );
    }
}
...
fn remove_rule() {
    ...
    let (selected_rule_id, selection) = {
        let rules = RULES.lock().unwrap();
        let rule_descriptions: Vec<String> =
...
fn remove_iptables_rule(rule_id: &str) {
    ...
    if output.status.success() {
        println!(
            "Successfully removed iptables rule for rule ID: {}",
            rule_id
        );
    } else {
